var EdicionCrias = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Catalogo';
	this.saveAndClose=false;
	
	this.configurarComboFk_camada=function(){
		var me=this;
		
		$('select[name="fk_camada"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_camada input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_camadaEnAjax) return true;			
			me.setDSFk_camada();
			me.Fk_camadaEnAjax=true;
		 });
	};
		
		
	this.setDSFk_camada = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/crias/buscar',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_camada"]').wijcombobox('option','data',datasource);
	};
		
	this.configurarComboFk_sexo=function(){
		var me=this;
		
		$('select[name="fk_sexo"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_sexo input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_sexoEnAjax) return true;			
			me.setDSFk_sexo();
			me.Fk_sexoEnAjax=true;
		 });
	};
		
		
	this.setDSFk_sexo = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/crias/buscarGenero',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_sexo"]').wijcombobox('option','data',datasource);
	};
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Cria: ' + getValorCampo('codigo') + ''); 
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_camada"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_camada"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_camada'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_camada'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_camada'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_sexo"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_sexo"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_sexo'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_sexo'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_sexo'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_camada();
this.configurarComboFk_sexo();
	};
