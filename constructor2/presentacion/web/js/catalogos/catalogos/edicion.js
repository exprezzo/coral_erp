var EdicionCatalogos = function(){
	...
	this.configurarComboFk_modulo=function(){
		var me=this;
		...
		$('select[name="fk_modulo"]').wijcombobox({			
		 ...
		 $('.contenedor_fk_modulo input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_moduloEnAjax) return true;			
			me.setDSFk_modulo();
			me.Fk_moduloEnAjax=true;
		 });
	};
		
		
	this.setDSFk_modulo = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/catalogos/buscarAplicacion',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_modulo"]').wijcombobox('option','data',datasource);
	};
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_modulo"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_modulo"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_modulo'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_modulo'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_modulo'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
		...
		//-----------------------------------
		var datos=paramObj;
		$(tabId+' .tabla_elementos').wijgrid('endEdit');
				var elementos=$(tabId+' .tabla_elementos').wijgrid('data');
				datos.elementosDeCatalogos = elementos;
		...
		$.ajax({
			...
			if ( resp.success == true	){
				...
				//--------------------
				var elementos=resp.datos.elementosDeCatalogos;	

				var grid=$(me.tabId+" .tabla_elementos");
