var EdicionElementos_de_catalogo = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Elemento de Catalogo';
	this.saveAndClose=false;
	
	this.configurarComboFk_componente=function(){
		var me=this;
		
		$('select[name="fk_componente"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_componente input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_componenteEnAjax) return true;			
			me.setDSFk_componente();
			me.Fk_componenteEnAjax=true;
		 });
	};
		
		
	this.setDSFk_componente = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/elementos_de_catalogo/buscarConmponente',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_componente"]').wijcombobox('option','data',datasource);
	};
		
	this.configurarComboFk_catalogo=function(){
		var me=this;
		
		$('select[name="fk_catalogo"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_catalogo input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_catalogoEnAjax) return true;			
			me.setDSFk_catalogo();
			me.Fk_catalogoEnAjax=true;
		 });
	};
		
		
	this.setDSFk_catalogo = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/elementos_de_catalogo/buscarCatalogo',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_catalogo"]').wijcombobox('option','data',datasource);
	};
		
	var me=this;
	this.borrar=function(){		
		var r=confirm("¿Eliminar Elemento de Catalogo?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Elemento de Catalogo: ' + getValorCampo('campo') + ''); 
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_componente"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_componente"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_componente'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_componente'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_componente'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_catalogo"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_catalogo"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_catalogo'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_catalogo'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_catalogo'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_componente();
this.configurarComboFk_catalogo();
	};
