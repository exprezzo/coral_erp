...
require_once $_PETICION->basePath.'/presentacion/html.php/elementos_de_catalogo/elemento_de_catalogo_pdf.php';

require_once $_PETICION->basePath.'/modelos/conmponente.php';

require_once $_PETICION->basePath.'/modelos/catalogo.php';

class elementos_de_catalogo extends Controlador{
	var $modelo="elemento_de_catalogo";	
	
	
		function buscarConmponente(){
			$conmponenteMod= new conmponenteModelo();
			$res = $conmponenteMod->buscar( array() );
			echo json_encode( $res );
		}
		
		function buscarCatalogo(){
			$catalogoMod= new catalogoModelo();
			$res = $catalogoMod->buscar( array() );
			echo json_encode( $res );
		}
	...
	function bajarPdf(){
		...
		//-------
		$objPdf = new Elemento_de_catalogoPdf('P','mm','letter');
		$objPdf->datos=$datos;
		$objPdf->AddPage();
		$objPdf->imprimir(  );
		//-------
	...
	function eliminar(){
		...
		if (empty($id) ){			
			...
			$response=array(
				'success'=>$res,
				'msg'=>'Elemento de Catalogo Eliminado'
			);
