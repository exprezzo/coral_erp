...
require_once $_PETICION->basePath.'/modelos/status_app.php';
require_once $_PETICION->basePath.'/presentacion/html.php/status_apps/status_app_pdf.php';

class status_apps extends Controlador{
	var $modelo="status_app";	
	...
	function bajarPdf(){
		...
		//-------
		$objPdf = new Status_appPdf('P','mm','letter');
		$objPdf->datos=$datos;
		$objPdf->AddPage();
		$objPdf->imprimir(  );
		//-------
	...
	function eliminar(){
		...
		if (empty($id) ){			
			...
			$response=array(
				'success'=>$res,
				'msg'=>'Status de Aplicacion  Eliminado'
			);
