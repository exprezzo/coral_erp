...
require_once $_PETICION->basePath.'/presentacion/html.php/aplicaciones_de_empresas/aplicacion_empresa_pdf.php';
require_once $_PETICION->basePath.'/modelos/empresa.php';
require_once $_PETICION->basePath.'/modelos/app.php';

class aplicaciones_de_empresas extends Controlador{
	var $modelo="aplicacion_empresa";	
	...
	function bajarPdf(){
		...
		//-------
		$objPdf = new Aplicacion_empresaPdf('P','mm','letter');
		$objPdf->datos=$datos;
		$objPdf->AddPage();
		$objPdf->imprimir(  );
		//-------
	...
	function eliminar(){
		...
		if (empty($id) ){			
			...
			$response=array(
				'success'=>$res,
				'msg'=>'Aplicacion de Empresa Eliminada'
			);
