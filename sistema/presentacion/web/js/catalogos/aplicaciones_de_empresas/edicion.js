var EdicionAplicaciones_de_empresas = function(){
	this.editado=false;
	this.tituloNuevo='Nueva Aplicacion de Empresa';
	...
	this.setDSFk_empresa = function(){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/aplicaciones_de_empresas/buscarEmpresa',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.setDSFk_app = function(){		
		...
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/aplicaciones_de_empresas/buscarApp',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.borrar=function(){		
		var r=confirm("¿Eliminar Aplicacion de Empresa?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Aplicacion de Empresa: ' + getValorCampo('id') + ''); 
		}else{
