var EdicionAplicaciones_del_usuario = function(){
	this.editado=false;
	this.tituloNuevo='Nueva Aplicacion Para el Usuario';
	...
	this.setDSFk_usuario = function(){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/aplicaciones_del_usuario/buscarUsuario',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.setDSFk_app = function(){		
		...
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/aplicaciones_del_usuario/buscarApp',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.borrar=function(){		
		var r=confirm("¿Desinstalar Aplicacion?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Aplicacion: ' + getValorCampo('fk_app') + ' -  ' + getValorCampo('fk_usuario') + ''); 
		}else{
